...
//					   				}
					   ...
					   TICK_EtherCAT_MasterCommand = {
					   				data[0]++;
//					   				if(data[0]==0) print("1");
//					   				else if(data[0]==1) print("1");
//					   				else if(data[0]==2) print("2");
//					   				else if(data[0]==3) {print("3"); data[0] = 0;}
//									print("data(ethercat) = ", data[0]);
//
//										for(i=0; i<7; i++)
//										{
//											print("pos ", i, " = ", pos[i]);
//										}
//										print("g_pos in ecat = ", g_pos);

									if(data[0]%1000 == 0) {USER_PARAM(5) = 1;}
									//if(data[0]==10) {USER_PARAM(5) = 6; USER_PARAM(5) = 8;}
					   				}
		               ...
		               SIG_PLAY	  = {
		                             ...
		                             //AxisPosRelStart(C_AXIS1, 1000);
									 //AxisWaitReached(C_AXIS1);
									 Cvel(C_AXIS1,data[0]/1000);
...
//				}
	TICK_Configure_SineWave = {
		cnt_sine_wave ++;
		pos[0] = SINE_WAVE_AMP * sin(SINE_WAVE_TIME*(1/100)*cnt_sine_wave);
		g_pos = SINE_WAVE_AMP * sin(SINE_WAVE_TIME*(1/100)*cnt_sine_wave);
		if(cnt_sine_wave%100 == 0)print("sin cnt = ", cnt_sine_wave, "/", g_pos);
    }
}
...
SmMachine Operation {1, init, MyMachine, 200, 2}
SmMachine TCPIP_Operation {2, init_tcp, TCPIP_Handler, 200, 2}
...
long main(void)
{
	...
	Dec(C_AXIS1,500);   // 감속도 지정
  	...
  	res = SmRun(Operation, TCPIP_Operation); // Opearation 이라는 이름의 Statemachine을 실행.

    return(0);
}

long init(long id, long data[])
{
	data[0] = 0;

	//SmPeriod(2, id, TICK_EtherCAT_MasterCommand);
	//SmTimer(1000, id, TICK_EtherCAT_MasterCommand);
	//SmPeriod(1, id, TICK_Configure_SineWave);

	return(0);
}

long init_tcp(long id2, long data[])
{
	data[0]=1;
	SmPeriod(1, id2, TICK_Configure_SineWave);
