...
//				}
	TICK_Configure_SineWave = {
		...
		if(cnt_sine_wave%SINE_WAVE_TIMER_FREQ == 0)print("sin cnt = ", cnt_sine_wave, "/", g_pos);
    }

    TICK_TCP_Receive = {
        // TCP read function will be filled in.
    }
    TICK_TCP_Send = {
        // TCP read function will be filled in.
    }
...
SmMachine M_EtherCAT {Machine_EtherCAT, init_EtherCAT, EtherCAT_Handler, 200, 20}
SmMachine M_TCP {Machine_TCP, init_tcp, TCPIP_Handler, 200, 20}
...
long main(void)
{
	...
	Dec(C_AXIS1,500);   // 감속도 지정
  	...
  	res = SmRun(M_EtherCAT, M_TCP); // Opearation 이라는 이름의 Statemachine을 실행.

    return(0);
}

long init_EtherCAT(long id, long data[])
{
	data[0] = 0;
