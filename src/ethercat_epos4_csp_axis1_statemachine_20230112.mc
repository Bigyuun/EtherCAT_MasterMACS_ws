...
//				}
	TICK_Configure_SineWave = {
		...
		print("sin cnt = ", cnt_sine_wave, "/", g_pos);
    }
}
...
SmMachine Operation {1, init, MyMachine, 20, 2}
SmMachine TCPIP_Operation {2, init_tcp, TCPIP_Handler, 20, 2}
...
long main(void)
{
	...
	sdkEpos4_SetupECatVirtAmp(C_AXIS1, C_AXIS1_MAX_RPM, EPOS4_OP_CSP);
	...
	Vel(C_AXIS1,500);   // 속도 지정
	Acc(C_AXIS1,1000);   // 가속도 지정
	Dec(C_AXIS1,1000);   // 감속도 지정
...
long init(long id, long data[])
{
	...
	SmPeriod(1000, id, TICK_EtherCAT_MasterCommand);
	//SmPeriod(1, id, TICK_Configure_SineWave);

	return(0);
}

long init_tcp(long id, long data[])
{
	data[0]=1;
	SmPeriod(10, id, TICK_Configure_SineWave);
